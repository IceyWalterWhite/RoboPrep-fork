...
export const parsedRoundSchema = z.object({
  roundNumber: z.coerce.number().int().min(1).max(20).nullish().transform((v) => v ?? null),
  title: nullableString,
  roundType: z.enum(ROUND_TYPES).catch("unknown").default("unknown"),
  durationMinutes: z.coerce.number().int().min(1).max(600).nullish().transform((v) => v ?? null),
...
export const parsedQuestionSchema = z.object({
  originalWording: z.string().trim().min(2, "original wording is required"),
  normalizedText: nullableString,
  questionType: z.enum(QUESTION_TYPES).nullish().transform((v) => v ?? null),
  roundNumber: z.coerce.number().int().min(1).max(20).nullish().transform((v) => v ?? null),
  orderIndex: z.coerce.number().int().min(0).default(0),
  difficulty: z.enum(DIFFICULTIES).nullish().transform((v) => v ?? null),
...
export const parsedInterviewSchema = z.object({
  ...
  questions: z.array(parsedQuestionSchema).min(1, "at least one question is required").max(200),
});
...
export function validateParsedInterview(raw: unknown): ParsedInterviewPayload {
  ...
  if (!result.success) {
    ...
    throw new Error(`parser output failed schema validation at ${path}: ${first?.message ?? "unknown"}`);
  }
