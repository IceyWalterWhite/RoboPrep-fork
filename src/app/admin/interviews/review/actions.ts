...
function message(error: unknown): string {
  return error instanceof Error ? error.message.slice(0, 300) : "发生未知错误。";
}
...
function back(submissionId: string, error?: string): never {
  ...
  if (error) {
    redirect(
      `/admin/interviews/review/${submissionId}?error=${encodeURIComponent(error)}`,
    );
  }
...
async function guard() {
  ...
  if (!admin) back("", "数据导入服务未配置。");
  return { viewer, admin };
}

function notAuthorized(): never {
  redirect("/admin/interviews/review?error=" + encodeURIComponent("无权执行此操作。"));
}
...
export async function retryParseAction(formData: FormData): Promise<void> {
  ...
  if (!submissionId || !jobId) back(submissionId ?? "", "缺少任务或投稿信息。");
  try {
...
export async function editDraftAction(formData: FormData): Promise<void> {
  ...
  try {
    ...
    if (!draft) back(submissionId, "没有可编辑的草稿。");
...
export async function editRoundAction(formData: FormData): Promise<void> {
  ...
  if (!submissionId || !roundDraftId) back(submissionId ?? "", "缺少必要标识。");
  try {
...
export async function editQuestionAction(formData: FormData): Promise<void> {
  ...
  if (!submissionId || !questionDraftId) back(submissionId ?? "", "缺少必要标识。");
  try {
...
export async function acceptMatchAction(formData: FormData): Promise<void> {
  ...
  if (!submissionId || !questionDraftId || !questionId)
    back(submissionId ?? "", "缺少必要标识。");
  try {
...
export async function createCanonicalAction(formData: FormData): Promise<void> {
  ...
  if (!submissionId || !questionDraftId) back(submissionId ?? "", "缺少必要标识。");
  if (!title) back(submissionId, "创建新的标准问题时必须填写标题。");
  try {
...
export async function publishAction(formData: FormData): Promise<void> {
  ...
  let slug = "";
  try {
    const result = await publishDraft(admin, submissionId, viewer.id, {
      companyId,
      positionId,
    });
    slug = result.slug;
