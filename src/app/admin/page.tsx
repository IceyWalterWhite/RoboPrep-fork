...
export default async function AdminDashboardPage() {
  ...
  const [openTasks, failedJobs, stuckJobs, published, statsFreshness, recentEvents] =
    await Promise.all([
      admin
        .from("review_tasks")
      ...
      admin
        .from("ingestion_jobs")
        .select("id, submission_id, error_code", { count: "exact" })
        .eq("status", "failed")
  ...
  return (
    <Container width="wide" className="py-10">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        />
        <MetricCard
          label="卡住的任务（运行超过 30 分钟）"
          value={String(stuckJobs.count ?? 0)}
          href="/admin/audit"
          attention={(stuckJobs.count ?? 0) > 0}
        />
        <MetricCard label="已发布面经" value={String(published.count ?? 0)} />
      </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          ...
          <p className="text-ink-secondary mt-2 text-sm">
            {statsUpdatedAt
              ? `最近刷新：${new Date(statsUpdatedAt).toLocaleString()}。`
              : "尚未刷新——导入种子数据后运行 pnpm refresh:companies。"}
          </p>
        </Card>
          ...
          <ul className="mt-2 flex flex-col gap-1 text-sm">
            {flags.map((entry) => (
              <li key={entry.flag} className="text-ink-secondary flex justify-between">
                ...
                >
                  {entry.enabled ? "开启" : "关闭"}（{entry.envKey}）
                </span>
              </li>
            ))}
          </ul>
        ...
        <Card className="p-5 lg:col-span-2">
          ...
          <ol className="mt-2 flex flex-col gap-1 text-xs">
            ...
            {(recentEvents.data ?? []).length === 0 && (
              <li className="text-ink-tertiary">暂无事件。</li>
            )}
          </ol>
        </Card>
      </div>
