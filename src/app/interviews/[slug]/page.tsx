...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${interview.company?.name ?? "Embodied AI"} ${interview.position?.title ?? "Interview"} — ${period}`,
    description: interview.summary ?? "A structured Embodied AI interview experience on RoboPrep.",
...
export default async function InterviewDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <Container width="wide" className="py-10 sm:py-14">
      <Breadcrumbs items={[{ label: "Interviews", href: "/interviews" }, { label: interview.company?.name ?? "Interview" }, { label: `${interview.position?.title ?? interview.title} — ${period}` }]} />
        ...
        <main className="min-w-0">
          ...
          <div className="mt-8 flex flex-col gap-10">
            ...
            <section aria-labelledby="rounds-heading" className="flex flex-col gap-8">
              ...
              {interview.rounds.map((round) => <InterviewRound key={`${round.id ?? "generated"}-${round.roundNumber}`} round={round} />)}
            </section>
            <InterviewTopics topics={interview.topics} />
            <RelatedInterviews interviews={related} />
          </div>
        </main>
        <aside className="min-w-0 lg:pt-1"><div className="lg:sticky lg:top-20"><InterviewSource source={interview.source} /></div></aside>
      </div>
    </Container>
  );
}
